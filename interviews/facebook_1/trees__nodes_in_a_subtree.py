...
def count_of_nodes(root, queries, s):
  ...
  '''Review this! Puzzled for 2 hrs...
  '''
  output = list()
  freq = dict()

  for q in queries:
    #find target with val==q[0]
    tree = list()
    tree.append(root)
    target = root
    while len(tree) > 0 and target.val != q[0]:
      target = tree.pop(0)
      tree.extend(target.children)
    #explore target subtree for frequency of s[node.val-1]==q[1]
