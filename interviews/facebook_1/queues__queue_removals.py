...
def findPositions(arr, x):
  ...
  output = list()
  queue = list(zip(arr, range(1, len(arr)+1)))

  # perform x iterations of the following:
  for i in range(x):
    # popped = pop y elements from front of queue; y= len(queue) if len(queue) <= x else x
    y = len(queue) if len(queue) <= x else x
    popped = queue[:y]
    queue = queue[y:]
    # largest = max(popped)
    vals, indexes = zip(*popped)
    largest = popped[vals.index(max(vals))]
    output.append(largest[1])
    # popped.remove(largest)
    popped.remove(largest)
    # for val in popped:
    for val, index in popped:
